...
static void GenericApp_SendsTheMessage(afIncomingMSGPacket_t *ptr);
    ...
    while ( MSGpkt )
    {
      switch ( MSGpkt->hdr.event )
      {
        case AF_INCOMING_MSG_CMD:
            ...
            // 发送数据
            GenericApp_SendsTheMessage(MSGpkt);
          break;
...
static void GenericApp_SendsTheMessage( afIncomingMSGPacket_t *ptr )
{
  ...
  switch(ptr->clusterId)
  {
    case GENERICAPP_CLUSTERID:
      osal_memcpy(buffer,ptr->cmd.Data,sizeof(buffer));
      if (osal_memcmp(buffer,"0123456789",sizeof("0123456789")))
      {
