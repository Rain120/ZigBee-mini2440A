...
uint16 GenericApp_ProcessEvent( uint8 task_id, uint16 events )
{
    ...
    while ( MSGpkt )
    {
      ...
      {    
         case ZDO_STATE_CHANGE:   // 判断网络状态是否改变
          ...
          {
            // 启动用户事件
            osal_set_event(GenericApp_TaskID, USER_EVENT_LED_FLASH);
          }  
  ...
  if (events & USER_EVENT_LED_FLASH)  // 判断是否是USER_EVENT_LED_FLASH
  {
      //HalLedBlink(HAL_LED_2, 5, 50, 500);
     osal_start_timerEx(GenericApp_TaskID, USER_EVENT_LED_FLASH, 2000);  
     if (ledFlag == 0)
     {
     ...
     return (events ^ USER_EVENT_LED_FLASH);
  }
